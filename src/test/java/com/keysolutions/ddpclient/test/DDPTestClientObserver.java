...
public class DDPTestClientObserver implements IDDPListener, Observer {
	...
	public void onDDPMessage(Map<String, Object> jsonFields) {
		...
		String messageType = (String) jsonFields.get(DDPClient.DdpMessageField.MSG);
		if (messageType == null) {
			// ignore {"server_id":"GqrKrbcSeDfTYDkzQ"} web socket messages per DDP spec
			return;
		}
		if (messageType.equals(DdpMessageType.ERROR)) {
			mErrorSource = (String) jsonFields.get(DdpMessageField.SOURCE);
			mErrorMsg = (String) jsonFields.get(DdpMessageField.ERRORMSG);
		}
		if (messageType.equals(DdpMessageType.CONNECTED)) {
			mDdpState = DDPSTATE.Connected;
			mSessionId = (String) jsonFields.get(DdpMessageField.SESSION);
		}
		if (messageType.equals(DdpMessageType.CLOSED)) {
		...
		if (messageType.equals(DdpMessageType.ADDED)) {
		...
		if (messageType.equals(DdpMessageType.REMOVED)) {
			String collName = (String) jsonFields.get(DdpMessageField.COLLECTION);
			if (mCollections.containsKey(collName)) {
		...
		if (messageType.equals(DdpMessageType.CHANGED)) {
